...
1. Трансформишите своје податке. Изаберите ентитет за уређивање или трансформисање. Користите опције у прозору Power Query да бисте применили трансформације. Свака трансформација је наведена у оквиру Примењених **корака**. Power Query пружа бројне [унапред изграђене опције](/power-query/power-query-what-is-power-query#transformations) трансформације.
   ...
   > - Уверите се да је тип података постављен на одговарајући начин и да се подудара са подацима. На пример, за поља типа датума изаберите тип датума.
...
### <a name="data-type-does-not-match-data"></a>Тип података се не подудара са подацима

До најчешће неподударања типа података долази када поље датума није подешено на исправан формат датума.

Подаци се могу поправити на извору и поново унети. Или поправите трансформацију у оквиру увида клијената. Да бисте поправили трансформацију:

1. Идите на **Подаци** > **Извори података**.

1. Поред ставке извор података оштећеним подацима изаберите **уреди**.

1. Изаберите **Следеће**.

1. Изаберите сваки упит и потражите трансформације примењене унутар "Примењених корака" које су нетачне или колоне датума које нису трансформисане са форматом датума.

   :::image type="content" source="media/PQ_corruped_date.png" alt-text="Power Query- Уређивање приказивања нетачног формата датума":::

1. Промените тип података да би се исправно подударали са подацима.

1. Изаберите **Сачувај**. Тај извор података је освежен.

## <a name="troubleshoot-ppdf-power-query-based-data-source-refresh-issues"></a>Решавање проблема са освежавање извор података ППДФ-а Power Query

Ако су подаци бајати или добијате грешке након освежавања извор података, извршите следеће кораке:

1. Идите на [Power Platform](https://make.powerapps.com)

1. Изаберите окружење **за** инстанцу увида купаца.

1. Крећите се **до датафлоwс**.

1. За ток података који одговара пољу за извор података увидима клијената изаберите вертикалну елипсу () и изаберите ставку Прикажи &vellip; историју **освежавања**.

1. Ако је **статус** прилива података "Успех **", власништво** над извор података Power Query се можда променило:

   1. Прегледајте распоред освежавања из историје освежавања.
   1. Подесите распоред новог власника и сачувајте поставке.

1. Ако статус **прилива** података није **успео**:

   1. Преузмите датотеку историје освежавања.
   1. Прегледајте преузету датотеку због пропуста.
   1. Ако није могуће разрешити грешку, изаберите **?** да бисте отворили тикет за подршку. Укључите преузету датотеку историје освежавања.
